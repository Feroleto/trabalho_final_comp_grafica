...
void spawnBezierProjectiles(Object* attacker, Object* target, Projectile* proj1) {
    proj1->isActive = true;
    proj1->currentFrame = 0;
    

    proj1->curve.p0 = attacker->transform.position;
    proj1->curve.p3 = target->transform.position;

    float distance = glm::distance(proj1->curve.p0, proj1->curve.p3);
    float speed = 0.15f;
    
    int frames = static_cast<int>(distance / speed);
    proj1->totalFrames = std::max(1, frames);
...
void updateBezier(Attack* attack, Projectile* proj1) {
    if (proj1->isActive) {
