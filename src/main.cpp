...
// Armazenamento das últimas matrizes da mão/espada para congelar durante game over
glm::mat4 g_CharacterRightHandMatrix = glm::mat4(1.0f);
glm::mat4 g_CharacterSwordModel = glm::mat4(1.0f);
glm::mat4 g_OpponentRightHandMatrix = glm::mat4(1.0f);
glm::mat4 g_OpponentSwordModel = glm::mat4(1.0f);
...
const float PLAYER_START_Z = 0.0f;
const float OPPONENT_START_X = 3.0f;
const float OPPONENT_START_Z = 0.0f;
...
void resetGame(LookAtCamera& camera) {
    ...
    g_CharacterZ = PLAYER_START_Z;
    g_OpponentX = OPPONENT_START_X;
    g_OpponentZ = OPPONENT_START_Z;
    g_CharacterCurrentAnimation = "idle";
    g_OpponentCurrentAnimation = "idle";
    g_CharacterAnimationTime = 0.0f;
    g_CharacterForcedAnimationEnd = 0.0f;
...
int main(int argc, char* argv[])
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (!g_GameOver) {
            ...
            if (!g_Proj1opponentSpawned && 
                g_OpponentCurrentAnimation == "triple_slash_attack" &&
                currentTime >= g_OpponentAnimationStartTime + delay1Seconds) {
                
                g_OpponentObject.transform.position = g_OpponentSwordWorldPos;
                g_OpponentObject.transform.dirty = true;

                g_TargetObject.transform.position = {g_CharacterX, g_CharacterY, g_CharacterZ};
                g_TargetObject.transform.dirty = true;
                
                spawnBezierProjectiles(&g_OpponentObject, &g_TargetObject, &g_Proj1opponent/*, &g_Proj2*/);
                g_Proj1opponentSpawned = true;
            }
            // Proj2
            ...
            if (!g_Proj2opponentSpawned && 
                g_OpponentCurrentAnimation == "triple_slash_attack" &&
                currentTime >= g_OpponentAnimationStartTime + delay2Seconds) {
                ...
                spawnBezierProjectiles(&g_OpponentObject, &g_TargetObject, &g_Proj2opponent/*, &g_Proj2*/);
                g_Proj2opponentSpawned = true;
            }
            ...
            if (!g_Proj3opponentSpawned && 
                g_OpponentCurrentAnimation == "triple_slash_attack" &&
                currentTime >= g_OpponentAnimationStartTime + delay3Seconds) {
                ...
                spawnBezierProjectiles(&g_OpponentObject, &g_TargetObject, &g_Proj3opponent/*, &g_Proj2*/);
                g_Proj3opponentSpawned = true;
            }
        }
        ...
        updateBezier(&g_SlashAttack, &g_Proj1opponent);
        updateBezier(&g_SlashAttack, &g_Proj2opponent);
        updateBezier(&g_SlashAttack, &g_Proj3opponent);
        ...
        if (g_Proj1opponent.isActive) {
            glActiveTexture(GL_TEXTURE0 + 3);
            glBindTexture(GL_TEXTURE_2D, g_ProjectileTextureID);

            glm::vec3 pos = g_Proj1opponent.hitbox.getGlobalPosition();
            glm::vec3 camDir = camera.getViewDirection();
        ...
        if (g_Proj2opponent.isActive) {
            glActiveTexture(GL_TEXTURE0 + 3);
            glBindTexture(GL_TEXTURE_2D, g_ProjectileTextureID);

            glm::vec3 pos = g_Proj2opponent.hitbox.getGlobalPosition();
        ...
        if (g_Proj3opponent.isActive) {
            glActiveTexture(GL_TEXTURE0 + 3);
            glBindTexture(GL_TEXTURE_2D, g_ProjectileTextureID);

            glm::vec3 pos = g_Proj3opponent.hitbox.getGlobalPosition();
